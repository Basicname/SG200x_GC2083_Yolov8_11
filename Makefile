...
ifndef SDK_ROOT_PATH
$(error "Please set tdl sdk root path to SDK_ROOT_PATH")
endif
ifndef MW_PATH
...
ifeq ($(MW_VERSION), v1)
    ifeq ($(CHIP), CV183X)
        MW_LIBS = -lini -lsns_full -lsample -lisp -lvdec -lvenc -lawb -lae -laf -lcvi_bin -lcvi_bin_isp -lcvi_vcodec -lsys -lcvitracer -lcvi_jpeg -lvpu -lcvi_rtsp
        MW_ISP_INC_PATH = $(MW_INC_PATH)/isp/cv183x
    else ifeq ($(CHIP), CV182X)
        MW_LIBS = -lini -lsns_full -lsample -lisp -lvdec -lvenc -lawb -lae -laf -lcvi_bin -lcvi_bin_isp -lcvi_vcodec -lsys -lcvitracer -lcvi_jpeg -lvpu -lisp_algo -lcvi_rtsp
        MW_ISP_INC_PATH = $(MW_INC_PATH)/isp/cv182x
    endif
else ifeq ($(MW_VERSION), v2)
    ifeq ($(CHIP), CV186X)
        ifndef KERNEL_ROOT
$(error "Please set linux kernel root path to KERNEL_ROOT")
        endif
        MW_LIBS = -lsys -lvi -lvpss -lvenc -lvdec -lvo -lrgn -lgdc -lini -lisp -lsns_full -lawb -lae -laf -lcvi_bin_isp -lcvi_bin -lisp_algo -lcvi_rtsp
        ...
        MW_LIBS = -lini -lsns_full -lsample -lisp -lvdec -lvenc -lawb -lae -laf -lcvi_bin -lcvi_bin_isp -lmisc -lisp_algo -lsys -lvpu -lcvi_rtsp
    ...
    ifeq ($(CHIP), CV181X)
        MW_LIBS = -lini -lmsg -lvpu -lcvilink -lsys -lvenc -lvdec -lcvi_bin -lcvi_bin_isp -lisp -lae -lawb -lae -laf -lcvi_rtsp
        MW_ISP_INC_PATH = $(MW_INC_PATH)/isp/cv181x/
...
SAMPLE_MW_LIB = -L$(MW_LIB_PATH) -L$(MW_3RD_LIB_PATH) $(MW_LIBS) -L$(SDK_3RD_LIB_PATH) -L$(RTSP_LIB_PATH)
SAMPLE_OPENCV_LIB = -L$(OPENCV_LIB_PATH) $(OPENCV_LIBS)

SAMPLE_INIT_LIBS = $(SAMPLE_MW_LIB) $(SAMPLE_BASE_LIBS)
SAMPLE_AUD_LIBS = $(SAMPLE_MW_LIB) $(SAMPLE_BASE_LIBS) $(MW_AUDIO_LIBS)
SAMPLE_YOLO_LIBS = $(SAMPLE_MW_LIB) $(SAMPLE_BASE_LIBS) $(SAMPLE_OPENCV_LIB)
SAMPLE_VI_LIBS = $(SAMPLE_MW_LIB) $(SAMPLE_OPENCV_LIB) $(SAMPLE_BASE_LIBS) -L$(RTSP_LIB_PATH) $(RTSP_LIBS)
SAMPLE_APP_LIBS = $(SAMPLE_MW_LIB) $(SAMPLE_OPENCV_LIB) $(SAMPLE_BASE_LIBS) -L$(RTSP_LIB_PATH) $(RTSP_LIBS) $(SDK_APP_LIBS)
...
$(info SAMPLE_MW_LIB: $(SAMPLE_MW_LIB))
